...
class Pointer implements Comparable<Pointer> {
    ...
    @Override
    public int compareTo(Pointer next) {
...
class Solution {
    ...
    public List<List<Integer>> verticalTraversal(TreeNode root) {
        ...
        int temp = points.get(0).x;
        
        for(Pointer ps : points ) {
            if(temp != ps.x) {
            ...
            results.get(results.size()-1).add(ps.value);
        }
    ...
    private void dfs(TreeNode node,int x, int y) {
        ...
        points.add(new Pointer(x,y,node.val));
